...
def safetensors_weights_iterator(
    ...
    *,
    ...
    """
    if os.environ.get("VLLM_ENGRAM_DISK_BACKED", "0").strip().lower() in {"1", "true", "yes", "on"}:
    ...
    if reader_contract is not None:
        from .exl3_attested_reader import ReaderContract
        if type(reader_contract) is not ReaderContract or safetensors_load_strategy != "lazy":
            raise ValueError("attested reader requires explicit validated contract and lazy strategy")
    elif descriptor_mode:
        raise ValueError("descriptor mode requires an attested reader contract")
    ...
    for st_file in tqdm(
        ...
        if safetensors_load_strategy == "eager":
            ...
            manager = (reader_contract.open(st_file, safe_open) if reader_contract is not None
                       else safe_open(st_file, framework="pt"))
            with manager as f:
                for name in f.keys():  # noqa: SIM118
                    if reader_contract is not None:
                        reader_contract.note("seen", name)
                    if (reader_contract is not None
                            and getattr(reader_contract, "draft_only", False)
                            and not str(name).startswith("mtp.")):
                        continue
                    if should_skip_weight(name, local_expert_ids) or (
                        reader_contract is not None and reader_contract.should_skip_trellis(name, local_expert_ids)
                    ):
                        if reader_contract is not None:
                            reader_contract.note("ep_skipped", name)
                        continue
                    ...
                    try:
                        ...
                        if should_skip_engram_embed_tensor(name):
                            if reader_contract is not None:
                                reader_contract.note("engram_skipped", name)
                            logger.info_once(
                        ...
                        pass
                    param = (reader_contract.descriptor(st_file, name) if descriptor_mode
                             else f.get_tensor(name))
                    yield name, param
                    del param

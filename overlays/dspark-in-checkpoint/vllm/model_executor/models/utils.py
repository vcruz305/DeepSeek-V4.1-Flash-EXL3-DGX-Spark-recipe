...
def get_draft_quant_config(vllm_config: VllmConfig) -> "QuantizationConfig | None":
    ...
    # DeepSeek-V4(.1) DSpark: the draft reuses the target checkpoint, and the
    # draft's own config.json carries only the base EXL3 declaration. Copy the
    # attested runtime override fields from the target's quant config so the
    # draft's attention gets the fp8 delegate (weight_scale_inv) and its MTP
    # experts stay in the source format instead of being re-quantized.
    if cfg is not None:
        target_cfg = getattr(vllm_config, "quant_config", None)
        if target_cfg is not None:
            for _field in (
                "non_routed_quantization",
                "non_routed_dtype_policy",
                "mtp_experts",
                "mtp_experts_start_layer",
            ):
                _tval = getattr(target_cfg, _field, None)
                if _tval not in (None, ""):
                    try:
                        setattr(cfg, _field, _tval)
                    except Exception:
                        pass
    return cfg

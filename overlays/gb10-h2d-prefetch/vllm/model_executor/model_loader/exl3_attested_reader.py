...
class ReaderContract:
    def __init__(self, root, revision, effective_quantization, recipe_root='/recipe'):
        ...
        if (model_lock['required_loader_contract'] != 'per_expert_mixed_k_routed_trellis'
                ...
                or revision != self.lock['model_revision']):
...
class AttestedWeightSource:
    ...
    def _materialize(self, plan):
        ...
        depth = max(1, int(__import__('os').environ.get('VLLM_EXL3_H2D_PREFETCH', '4')))
...
def contract_for_model(loader, model_config):
    ...
    contract = getattr(loader, '_attested_reader_contract', None)
    if contract is None:
        contract = ReaderContract(model_config.model, model_config.revision,
                                  model_config.hf_config.quantization_config)
        loader._attested_reader_contract = contract
    elif (contract.root != Path(model_config.model).resolve()
          or contract.lock['model_revision'] != model_config.revision
          or contract.attestation['runtime_hf_overrides']['quantization_config']
             != model_config.hf_config.quantization_config):
        raise ValueError('attempt to reuse attested metadata for a different model/override')
